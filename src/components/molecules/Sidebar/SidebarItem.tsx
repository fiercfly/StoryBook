...
const SidebarItem: FC<SidebarItemProps> = (item) => {
	...
	const handleMainItemClick = (event: React.MouseEvent) => {
		...
		if (hasChildren) {
			...
			// Always navigate to the parent URL if it exists, for better UX
		...
		return (
			<SidebarMenuItem className={cn(isCollapsed && 'mb-3')}>
				<SidebarMenuButton
					...
					className={cn(
						'flex items-center gap-2 h-10 px-2 py-[10px] rounded-[6px] text-[14px] cursor-pointer transition-all duration-200 ease-in-out',
						isMainItemActive ? 'bg-zinc-100 font-medium' : 'font-normal text-muted-foreground hover:bg-zinc-100/50',
