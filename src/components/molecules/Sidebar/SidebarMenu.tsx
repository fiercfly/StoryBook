...
const SidebarNav: FC<{ items: NavItem[] }> = ({ items }) => {
	...
	useEffect(() => {
		...
		const isOnStandaloneItem = standaloneItems.some((item) =>
			item.url === '/' ? location.pathname === '/' : location.pathname.startsWith(item.url) && item.url !== '#',
		);
		...
		for (const item of items) {
			if (item.items && item.items.length > 0) {
				const isMainItemActive = item.url === '/' ? location.pathname === '/' : location.pathname.startsWith(item.url) && item.url !== '#';
